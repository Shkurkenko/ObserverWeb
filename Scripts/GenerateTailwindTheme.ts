// NOTE: Valid for older then 4-th version of tailwind where we could set color aliases through tailwind.config.ts theme->color section
...
function generateTailwindColorsFile() {
  ...
  const lines = [
    `//${GEN_THEMES_OUT_DIR}/${TAILWIND_COLORS_FILE_NAME}`,
    `// Автоматически сгенерировано — НЕ РЕДАКТИРОВАТЬ ВРУЧНУЮ!`,
    `// Запуск: npm run generate:themes`,
    ``,
    `const forensicTailwindColorScheme = {`,
  ]

  kebabTokens.forEach((token: string) => {
    const varName = `--md-sys-color-${token.replace(/[A-Z]/g, (m) => '-' + m.toLowerCase())}`
    lines.push(`  "${token}": "rgb(var(${varName}) / <alpha-value>)",`)
  })

  lines.push(`};`)
  lines.push(``)
  lines.push(`export default forensicTailwindColorScheme;`)

  fs.writeFileSync(TAILWIND_COLORS_FILE_NAME, lines.join('\n'))
  console.log(`Generated: ${TAILWIND_COLORS_FILE_NAME}`)
}
