...
export interface IGridProps extends Omit<IBoxProps, 'as'> {
  ...
  // Адаптивные колонки (опционально)
...
export const Grid: FunctionalComponent<IGridProps> = ({
  ...
  const classes = cn(
    ...
    sm && responsiveColumnClasses.sm[sm],
    md && responsiveColumnClasses.md[md],
    lg && responsiveColumnClasses.lg[lg],
    xl && responsiveColumnClasses.xl[xl],
    gapClasses[gap],
    className,
  )

  return (
    <Box as='div' className={classes} {...props}>
      {children}
    </Box>
  )
}
